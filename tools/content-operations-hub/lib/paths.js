...
export function isAppRoutePath(path) {
  ...
  return (
    normalized.includes("content-operations-hub") &&
...
export function sourcePathToDaDeletePath(sourcePath, helixPath = "") {
  const raw = normalizeFolderPath(
    String(sourcePath || "").replace(/\.html$/i, ""),
  );
...
export function helixPathToDaDocumentPath(helixPath, sourcePath) {
  ...
  if (web && web !== "/") {
    return (
      normalizeFolderPath(web.replace(/^\//, "").replace(/\.html$/i, "")) ||
      "index"
    );
  }
  if (web === "/") return "index";

  const fromSource = normalizeFolderPath(
    String(sourcePath || "").replace(/\.html$/i, ""),
  );
...
export function formatPageListLabel(helixPath, _name, browseFolder = "") {
  return {
    title: pageListRelativePath(helixPath, browseFolder),
    subtitle: null,
  };
}
