...
/**
 * Loads and runs the page template JS/CSS if a template metadata is set.
 * @param {Element} doc The document element
 */
async function loadTemplate(doc) {
  let template = getMetadata('template');
  // Auto-detect stories-article template when hero-article block is present
  if (!template && doc.querySelector('.hero-article')) {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`failed to load template ${template}`, err);
  }
}
