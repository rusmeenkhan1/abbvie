    ...
    if (link && subUl) {
      ...
      btn.addEventListener('click', () => {
        if (isDesktop.matches) {
          ...
          const currentlyOpen = navSections.querySelector('.nav-item.active');

          // Switching between menus — close previous without animation
          if (currentlyOpen && currentlyOpen !== li) {
            const prevPanel = currentlyOpen.querySelector('.mega-panel');
            if (prevPanel) {
...
export default async function decorate(block) {
  ...
  const sections = [...nav.children].filter((child) => child.classList?.contains('section'));
  classes.forEach((c, i) => {
